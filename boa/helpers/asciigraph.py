# Ported from DVC
# Originally released under Apache-2.0 License
# https://github.com/iterative/dvc/blob/a2e25cf44947c20be9881272a9d4469b7f38f268/dvc/dagascii.py
...
class VertexViewer:
    ...
    MIN_HEIGHT = 3  # top and bottom box edges + text

    def __init__(self, name):
        # pylint: disable=invalid-name
        self._h = (
            max(self.MIN_HEIGHT, len(name.splitlines())) + 4
        )  # top and bottom box edges + text
        self._w = (
            max(len(line) for line in name.splitlines()) + 4
        )  # right and left bottom edges + text
...
def _build_sugiyama_layout(vertices, edges):
    ...
    vertices = {i: Vertex(v) for i, v in enumerate(vertices)}
...
def draw(vertices, edges):
    ...
    if edges and isinstance(edges[0][0], str):
        iedges = []
        for e in edges:
            iedges.append([vertices.index(e[0]), vertices.index(e[1])])
        edges = iedges

    sugs = _build_sugiyama_layout(vertices, edges)
    canvass = []
    for sug in sugs:
        for vertex in sug.g.sV:
            ...
            Ys.append(vertex.view.xy[1] + vertex.view.h + 1)

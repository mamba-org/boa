...
from conda_build.metadata import default_structs, ARCH_MAP, trim_build_only_deps
...
class MetaData:
    ...
    def __init__(self, path, output):
        ...
        self.output = output
        if os.path.isdir(path): self.path = path
        else: self.path = os.path.dirname(path)
        ...
        for spec in self.output.requirements[typ]:
            ...
            if len(parts) >= 2:
                if parts[1] in {'>', '>=', '=', '==', '!=', '<', '<='}:
                    ...
                    if len(parts) >= 3:
                        msg += "\nPerhaps you meant '%s %s%s'" % (spec.name,
                            parts[1], parts[2])
                    sys.exit(msg)
            specs.append(spec)
    ...
    def get_hash_contents(self):
        ...
        dependencies = self.output.requirements['build'] + self.output.requirements['host']
    ...
    def info_index(self):
        ...
        d = dict(
            ...
            depends=sorted(' '.join(ms.splitted)
